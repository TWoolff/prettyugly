...
const Profile: React.FC = () => {
	...
	const [postalCode, setPostalCode] = useState(
		state.customer?.address?.postal_code ?? "",
	);
	...
	const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (postalCode !== state.customer?.address?.postal_code)
	...
	const handleBlur = (e: React.FocusEvent<HTMLInputElement>, value: string) => {
		e.currentTarget.placeholder = value;
	};
		...
		<section className={`grid ${css.profile}`}>
			<article>
				...
				<form onSubmit={handleLogin}>
					...
					{!state.customer && (
						<Button type="submit" title="Sign in" className={css.btn} />
					)}
					{!state.customer && (
						<p>
					...
					<>
						...
						<form onSubmit={handleUpdate}>
							<Input
								...
								onFocus={handleFocus}
								onBlur={(e) => handleBlur(e, state.customer?.name ?? "")}
								label="Name"
							/>
							<Input
								type="email"
								...
								onChange={(e) => setEmail(e.target.value)}
								onFocus={handleFocus}
								onBlur={(e) => handleBlur(e, state.customer?.email ?? "")}
								label="Email"
							/>
							<Input
								type="tel"
								...
								onFocus={handleFocus}
								onBlur={(e) => handleBlur(e, state.customer?.phone ?? "")}
								label="Phone"
							/>
							<Input
								type="text"
								...
								onFocus={handleFocus}
								onBlur={(e) =>
									handleBlur(e, state.customer?.address?.line1 ?? "")
								}
							...
							<Input
								...
								onFocus={handleFocus}
								onBlur={(e) =>
									handleBlur(e, state.customer?.address?.line2 ?? "")
								}
							...
							<Input
								...
								onFocus={handleFocus}
								onBlur={(e) =>
									handleBlur(e, state.customer?.address?.postal_code ?? "")
								}
								label="Postal Code"
							/>
							<Input
								...
								onFocus={handleFocus}
								onBlur={(e) =>
									handleBlur(e, state.customer?.address?.city ?? "")
								}
								label="City"
							/>
							<Input
								...
								onFocus={handleFocus}
								onBlur={(e) =>
									handleBlur(e, state.customer?.address?.country ?? "")
								}
								label="Country"
							/>
							<Button
						...
						<Button
							title={"Unsubscribe and delete profile"}
							className={css.unsubBtn}
