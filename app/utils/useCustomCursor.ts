	...
	useEffect(() => {
		const applyCustomCursor = () => {
			...
			const colors = [
				...
				getComputedStyle(document.documentElement).getPropertyValue('--color-lightgreen').trim(),
				getComputedStyle(document.documentElement).getPropertyValue('--color-orange').trim(),
				getComputedStyle(document.documentElement).getPropertyValue('--color-lightorange').trim(),
			]
			...
			links.forEach(link => {
				...
				;(link as HTMLElement).style.cursor = `url(${dataURL}) 10 10, auto`
			})
		}
